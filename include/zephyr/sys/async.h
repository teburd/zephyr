...
/**
 * Initialize a struct async_sem
 *
 * @param _sem Semaphore
 */
#define ASYNC_SEMAPHORE_INITIALIZER(_sem)                                      \
	{                                                                      \
		.result = 0, .sem = _sem                                       \
	}
...
/**
 * Initialize a struct async_mutex
 *
 * @param _mutex Pointer to a _mutex
 */
#define ASYNC_MUTEX_INITIALIZER(_mutex)                                        \
	{                                                                      \
		.result = 0, .mutex = _mutex                                   \
	}
...
/**
 * Initialize a struct async_work
 *
 * @param _work Work
 * @param _work_q Optional work queue to submit work to, NULL implies global
 * queue
 */
#define ASYNC_WORK_INITIALIZER(_work, _work_q)                                 \
	{                                                                      \
		.result = 0, .work = _work, .work_q = _work_q                  \
	}
 ...
 * the global work queue if @ref async_work.work_q is NULL.
 *
 * @funcprops \isr_ok
 *
 * @param context Optional context from which the callback originated,
