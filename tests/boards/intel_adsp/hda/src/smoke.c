...
void test_hda_smoke(void)
{
	 ...
	 */
	WAIT_FOR(cavs_ipc_send_message(CAVS_HOST_DEV, IPCCMD_HDA_INIT, STREAM_ID| (FIFO_SIZE << 8)));
	k_msleep(1);
	...
	WAIT_FOR(cavs_ipc_send_message(CAVS_HOST_DEV, IPCCMD_HDA_START, STREAM_ID));
	...
	WAIT_FOR(cavs_ipc_send_message(CAVS_HOST_DEV, IPCCMD_HDA_RESET, STREAM_ID));
