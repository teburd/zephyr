...
RTIO_MEMPOOL_ALLOCATOR_DEFINE(block_pool_alloc, 16, 1024, 1);
...
void test_rtio_mempool_allocator(void)
{
	...
	int res = rtio_block_alloc(&block_pool_alloc, &block, size, K_NO_WAIT);
	...
	res = rtio_block_alloc(&block_pool_alloc, &block2, size, K_NO_WAIT);

	zassert_equal(res, -ENOMEM, "Allocation should fail with no memory");

	rtio_block_free(&block_pool_alloc, block);
...
RTIO_MEMPOOL_ALLOCATOR_DEFINE(pipe_pool, 16, 1024, 1);
...
void sink_entry(void *fifov, void *p2, void *p3)
{
	...
	rtio_block_free(&pipe_pool, block);
}
