...
{
	void *tr_buf = z_soc_uncached_ptr((void *)(HP_SRAM_WIN3_BASE + HP_SRAM_WIN3_SIZE/2));
	void *lg_buf = z_soc_uncached_ptr((void *)HP_SRAM_WIN3_BASE);

	winstream_tr = sys_winstream_init(tr_buf, HP_SRAM_WIN3_SIZE/2);
	winstream_lg = sys_winstream_init(lg_buf, HP_SRAM_WIN3_SIZE/2);
}
