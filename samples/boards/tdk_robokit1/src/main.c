...
void fifo_stream(const struct device *icm42688, struct rtio *r, uint32_t n_bufs)
{
	...
	/* checkout the stream, this implies a FIFO setup */
	sensor_iodev_checkout(icm42688, &iodev);
	...
	/* Start the stream */
	sensor_iodev_start(icm42688, iodev);
	...
	/* All done streaming */
	sensor_iodev_checkin(icm42688, iodev);
