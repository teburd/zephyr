...
static inline void i2c_msg_from_rx(const struct rtio_sqe *iodev_sqe,
				   struct i2c_msg *msg)
{
	...
	msg->flags =
		((iodev_sqe->iodev_flags & RTIO_IODEV_I2C_STOP) ? I2C_MSG_STOP : 0) |
		((iodev_sqe->iodev_flags & RTIO_IODEV_I2C_RESTART) ? I2C_MSG_RESTART : 0) |
		((iodev_sqe->iodev_flags & RTIO_IODEV_I2C_10_BITS) ? I2C_MSG_ADDR_10_BITS : 0) |
		I2C_MSG_READ;
}

static inline void i2c_msg_from_tx(const struct rtio_sqe *iodev_sqe,
				   struct i2c_msg *msg)
{
...
static inline void i2c_msg_from_tiny_tx(const struct rtio_sqe *iodev_sqe,
					struct i2c_msg *msg)
{
...
void i2c_iodev_submit_work_handler(struct rtio_sqe *txn_first)
{
	const struct i2c_dt_spec *dt_spec = (const struct i2c_dt_spec *) txn_first->iodev->data;
