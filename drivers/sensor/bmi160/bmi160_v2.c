...
static int bmi160_read_data(const struct device *sensor, uint32_t sensor_type, void *out)
{
	struct bmi160_data *data = sensor->data;
	struct sensor_three_axis_data *out_data = out;
	int rc;
	uint16_t *sample;

	out_data->header.base_timestamp = k_uptime_get() * USEC_PER_MSEC;
...
static int bmi160_set_fifo_data_buffer(const struct device *sensor, struct sensor_raw_data *buffer)
{
	struct bmi160_data *data = sensor->data;
...
static int bmi160_set_process_data_callback(const struct device *sensor,
...
#ifdef CONFIG_SENSOR_STREAMING_MODE
	.set_fifo_data_buffer = bmi160_set_fifo_data_buffer,
	.set_process_data_callback = bmi160_set_process_data_callback,
...
#define BMI160_DEVICE_INIT(inst)                                                                   \
	DEVICE_DT_INST_DEFINE(inst, bmi160_init, NULL, &bmi160_data_##inst, &bmi160_cfg_##inst,    \
			      POST_KERNEL, CONFIG_SENSOR_INIT_PRIORITY, &bmi160_api);
