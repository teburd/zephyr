	...
	do { \
		...
		if ((HDA_DBG_SRC == HDA_DBG_BOTH) || (src == HDA_DBG_BOTH) || \
			...
			if (dbg_data->cur_sample < HDA_DMA_PTR_DBG_NUM_CP) { \
				...
				LOG_INFO("hda-dma-ptr-trace %08X %08X %08X " \
					postfix, info, wp, rp); \
				++dbg_data->cur_sample; \
			} \
		} \
...
static int cavs_hda_dma_start(struct device *dev, uint32_t channel)
{
	uint32_t flags;
	uint32_t dgcs;
	int ret = 0;
	...
	LOG_DBG("hda-dmac: %d channel %d -> start",
	       channel->dma->plat_data.id, channel->index);
	...
	if (channel->status != COMP_STATE_PREPARE || (dgcs & DGCS_GEN)) {
		ret = -EBUSY;
		LOG_ERR("hda-dmac: %d channel %d busy. dgcs 0x%x status %d",
...
static int dma_cavs_hda_stop(struct device *dev, uint32_t channel)
{
	struct hda_chan_data *hda_chan = dma_chan_get_data(channel);
	uint32_t flags;

	LOG_DBG("hda-dmac: %d channel %d -> release",
	       channel->dma->plat_data.id, channel->index);
...
static int dma_cavs_hda_stop_common(struct dma_chan_data *channel)
{
...
static int dma_cavs_hda_config(const struct device *dev, uint32_t channel, struct dma_config *config)
{

}

static int dma_cavs_hda_start(const struct device *dev, uint32_t channel)
{
	return 0;
}
...
static const struct dma_driver_api cavs_hda_dma_driver_api = {
	...
	.start = dma_cavs_hda_transfer_start,
	.stop = dma_cavs_hda_transfer_stop,
};
...
.ctx = {
.magic = DMA_MAGIC,
	.dma_channels = DT_INST_PROP(inst, channels),
